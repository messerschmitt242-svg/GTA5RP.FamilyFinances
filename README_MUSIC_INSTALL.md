...
## Ограничение музыкальных команд по каналу

Добавлена переменная окружения:

```env
CHANNEL_MUSIC=ID_ТЕКСТОВОГО_КАНАЛА_МУЗЫКИ
```

Теперь все команды `/music ...` и кнопки музыкальной панели работают только в этом текстовом канале. В других каналах бот отвечает предупреждением и не выполняет действие.
